...
if __name__=="__main__":    
    try: 
        ...
        df=read_format_csv(f"{sheet_path}/Meu_Municipio_Cobertura.csv")
        print(">>CSV file")
        ...
        # Altera tipo de dado
        df=change_type(df, 'Moradores_Cobertos', 2)
        df=change_type(df, 'Domicilios_Cobertos', 2)
        df=change_type(df, 'Area_km2_Coberta', 2)
        df=change_type(df, 'Moradores_Municipio', 1)       
        df=change_type(df, 'Domicilios_Municipio', 1)
        df=change_type(df, 'Area_Municipio_km2', 2)
        ...
        create_parquet(df, f"{parquet_path}/municipio_cobertura")
